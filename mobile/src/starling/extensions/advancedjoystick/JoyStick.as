package starling.extensions.advancedjoystick
{
	...
	public class JoyStick extends Sprite
	{
									 ...
									 activateImmediately:Boolean = true):void
		{
			...
			stick.pivotX = stick.width >> 1;
			stick.pivotY = stick.height >> 1;
			...
			pivotX = holder.width >> 1;
			pivotY = holder.height >> 1;
			maximumVelocityX = pivotX;// holder.width >> 1;
			maximumVelocityY = pivotY;// holder.height >> 1;
			_minOffsetX = (holder.width + stick.width) >> 1; 
			_minOffsetY = (holder.height + stick.height) >> 1; 
		...
		{
			_touched = false;
			stick.x = holder.width >> 1;
			stick.y = holder.height >> 1;
		}		
		...
		private function moveJoystick( touchX:Number, touchY:Number ):void
		{
			...
			if( Math.abs(dis) > pivotX )
			{
				...
				//trace(distX, distY, "dis", dis, "stick", stick.x, stick.y, "pivot", pivotPoint, force);
			}
		}
